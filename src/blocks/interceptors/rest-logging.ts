    ...
    return next.handle().pipe(
      catchError(err => {
        if (err instanceof HttpException) {
          ...
          logger[level]({ ...params, duration: this.fromStarted(startedAt), err }, `Request finished with error`);
          return throwError(() => err);
        }
        logger.error({ ...params, duration: this.fromStarted(startedAt), err }, `Request finished with error`);
